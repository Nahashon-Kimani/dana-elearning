    ...
    <hr>
       <div class="card">
         <div class="card-body">
           ...
           <hr>
            <form action="{{ route('lecturer.scheme.update', $scheme->id) }}" method="POST">
                @csrf
                @method('PUT')
             <div class="form-group">
              <label for="input-13">Associated Unit</label>
              <select name="unit_id" class="form-control form-control-square">
                <option selected value="{{ $scheme->unit_id }}">{{ $scheme->units->name }}</option>
             ...
             <div class="form-group">
                 <a href="{{ route('lecturer.scheme.index') }}" class="btn btn-danger shadow-danger btn-square px-5">Cancel</a>

...
<div class="row pt-2 pb-2">
 ...
 <div class="row">
    ...
    <hr>
       <div class="card">
         <div class="card-body">
           <div class="card-title text-uppercase">update <span class="text-info">{{ $unit->name }}</span> unit</div>
           <hr>
            <form action="{{ route('admin.unit.update', $unit->id) }}" method="POST">
                ...
                <div class="form-row">
                    ...
                    <input type="text" name="name" class="form-control form-control-square" value="{{ $unit->name }}">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group col-sm-5">
                        ...
                        <input type="text" name="duration" class="form-control form-control-square" value="{{ $unit->duration }}">
                       </div>
                       ...
                       <div class="form-group col-sm-7">
                        <label for="basic-select">Select Course</label>
                          <select name="course_id" class="form-control" class="form-control form-control-square" id="basic-select">
                                <option selected value="{{ $unit->course_id }}">{{ $unit->courses->name }}</option>
                ...
                <div class="form-row">
                    <div class="form-group col-sm-6">
                        <label for="input-15">Syllabus</label>
                            <select name="syllabus_id" class="form-control" class="form-control form-control-square" id="basic-select">
                                @if ($unit->syllabus_id == NULL)
                                    <option selected disabled>-- Select Unit Syllabus --</option>
                                @else
                                    <option value="$unit->syllabus_id" selected>{{ $unit->name }}</option>
                                @endif
                                <option disabled>_______________________</option>
                                @if (count($syllabi)==0)
                    ...
                    <div class="form-group col-sm-6">
                        <label for="input-15">Schemes</label>
                            <select name="scheme_id" class="form-control" class="form-control form-control-square" id="basic-select">
                                @if ($unit->scheme_id == NULL)
                                    <option selected disabled>-- Select Scheme of Work --</option>
                                @else
                                    <option value="{{ $unit->scheme_id }}">{{ $unit->name }}</option>
                                @endif
                                <option disabled>_______________________</option>
                                @if (count($schemes)==0)
                                    <option disabled>-- No Approved Scheme --</option>
                                @else
                ...
                <br><hr>
             <p class="text-uppercase">Unit Description and objectives (what the unit entails)</p>
             <div class="form-group">
              {{-- <label for="input-15">Unit Description and objectives (what the unit entails)</label> --}}
              <textarea name="desc" id="summernote" cols="30" rows="10">{{ $unit->desc }}</textarea>
             </div>
